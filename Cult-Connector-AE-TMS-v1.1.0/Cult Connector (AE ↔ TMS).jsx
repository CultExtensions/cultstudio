...
(function CultConnectorAE_Plugin(thisObj){
  ...
  /** Placeholder license header — server resolves workspace license on first TMS OAuth login. */
  ...
  // Publish releases on GitHub with a .jsxbin (or .jsx) asset attached to each tag (e.g. v1.1.0).
  var UPDATE_GITHUB_REPO  = "cult-studio";
  ...
  var SNAPSHOT_MARKER_COMMENT = "Crowdin";
  ...
  function oauthPollBodyDone(body) {
    return /"done"\s*:\s*true/i.test(String(body || ""));
  }
  ...
  /** Non-blocking OAuth poll via app.scheduleTask — avoids macOS system-wide wait cursor during browser login. */
  function oauthPollTick() {
    var ctx = $.global.CultConnectorAE_oauthCtx;
    if (!ctx || !ctx.active) return;
    try {
      var now = (new Date()).getTime();
      if ((now - ctx.startMs) >= ctx.timeout) {
        ctx.active = false;
        if (TMS_PROVIDER === "phrase" && EP_PHRASE_OAUTH_CONNECTED) {
          var last = curlGet(EP_PHRASE_OAUTH_CONNECTED);
          if (last.http === "200" && oauthPollBodyPhraseConnected(last.body)) {
        ...
        alertIf(tmsDisplayName() + " login timed out.\nIf the browser already shows Connected, click Connect once more—the panel will pick up the session.\nIf you closed the browser without finishing login, click Connect to start again.");
        if (ctx.onDone) ctx.onDone(false);
        $.global.CultConnectorAE_oauthCtx = null;
        return;
      }
      var s = curlGet(EP_OAUTH_STATUS + "?state=" + encodeURIComponent(ctx.state));
      if (s.body && oauthPollBodyDone(s.body)) {
        ctx.active = false;
        STATE.connected = true;
        ctx.setStatus("Connected.");
        if (ctx.onDone) ctx.onDone(true);
  ...
  // OAuth connect: crowdin mode "team"|"enterprise", or Phrase (mode ignored). onDone(success) when poll finishes.
  function oauthConnect(setStatus, mode, onDone){
    ...
    openUrl(url);
    setStatus("Waiting for authorization… (use the browser; After Effects stays responsive)");
    app.scheduleTask("$.global.CultConnectorAE_oauthPollTick()", 1500, false);
    return true;
  }
  ...
  function loadProjects(setStatus){
    ...
    if (r.http !== "200") {
      ...
      if (r.http === "401" && bodyStr.indexOf('"not_connected"') !== -1) {
  ...
  /** Always full comp: all text layers in comp and nested precomps. Selection is ignored; we only send entire compositions. */
    ...
    try {
      ...
      if (allowSnapshotMarker) {
        ...
        if (snapshotTime != null && isFinite(snapshotTime) && snapshotTime >= aNum - 0.001 && snapshotTime <= bNum + 0.001) {
          if (rangeRevealTime != null && isFinite(rangeRevealTime) && snapshotTime < rangeRevealTime - 0.001) {
  ...
  function findBestTime(layer, comp){
    try{
      ...
      if (tSnap != null && isFinite(tSnap) && tSnap >= a - 0.001 && tSnap <= b + 0.001) {
        if (rangeRevealBest != null && isFinite(rangeRevealBest) && tSnap < rangeRevealBest - 0.001) tSnap = null;
      }
      if (tSnap != null && layerEligible(layer, tSnap)) {
  ...
  function smartScanTimeline(setStatus, setProgress, compOptional, exportOnlyCollector){
    ...
    for (var ci = 0; ci < candidates.length; ci++) {
      ...
      var revealMinMain = null;
      if (c.layerComp === comp) {
        preferredTimesMain = getPreferredScreenshotTimes(c.layer, layerInMain, layerOutMain, true);
        revealMinMain = getTimeWhenRangeSelectorAnimatorReveal(c.layer, layerInMain, layerOutMain);
      } else {
        ...
        var rinPref = Number(c.layer.inPoint) || 0;
        var routPref = Number(c.layer.outPoint) || compEnd;
        var prefsPre = getPreferredScreenshotTimes(c.layer, rinPref, routPref, true);
        for (var pp = 0; pp < prefsPre.length; pp++) preferredTimesMain.push(prefsPre[pp] + sumStartPref);
        var revealPre = getTimeWhenRangeSelectorAnimatorReveal(c.layer, rinPref, routPref);
        if (revealPre != null && isFinite(revealPre)) revealMinMain = revealPre + sumStartPref;
      }
    ...
    if ((typeof EXPORT_ONLY_AND_MANIFEST !== "undefined" && EXPORT_ONLY_AND_MANIFEST) || (IS_WIN && exportOnlyCollector && typeof exportOnlyCollector.push === "function")) {
      ...
      for (var gi = 0; gi < groupKeys.length; gi++) {
        ...
        for (var bi = 0; bi < group.length; bi++) {
          ...
          var ssBase = safeScreenshotBase(STATE.fileKey) + "__" + safeGroupKey + "__" + safeId + "__t" + Math.round(tExport * 1000);
          var ssNameForLayer = crowdinScreenshotName(ssBase);
          var fBoxes = new File(tempPath("ct_boxes_" + TS + "_" + bi + ".json"));
    ...
    for (var gi = 0; gi < groupKeys.length; gi++) {
      ...
      // Sequential blocking curl per layer (tunnel-safe multipart).
      if (uploadCommands.length > 0) {
        scanDebugLog("Before sequential upload pngExists=" + pngFile.exists + " path=" + pngFile.fsName);
        for (var ui = 0; ui < uploadCommands.length; ui++) {
          if (tFirstUploadStart === null) tFirstUploadStart = (new Date()).getTime();
          scanDebugLog("Run upload " + ui + " (sequential curl)");
          ...
          var codeSq = (cSq && cSq.length > 0) ? cSq[0] : "0";
          var bodySq = (bSq && bSq.length > 0) ? bSq[0] : "";
          if (codeSq !== "200") {
            $.sleep(400);
            var resRq = runParallelScanUploads([seqCmdObj], { keepBodies: true });
            var cRq = resRq.codes || resRq;
            var bRq = resRq.bodies || [];
            if (cRq && cRq.length > 0) codeSq = cRq[0];
            if (bRq && bRq.length > 0) bodySq = bRq[0];
          }
    ...
    setStatus("Timeline Scan complete. " + okCount + " string(s), " + groupKeys.length + " frame(s).");
    if (setProgress) setProgress(layerComps.length, layerComps.length, "Complete");
    return okCount > 0;
  } finally {
  ...
  function applyTranslationsToComp(dupComp, map, originalComp) {
    ...
    var n = Math.min(origList.length, dupList.length);
    for (var i = 0; i < n; i++) {
      ...
      var sp = getSourceTextProp(dupList[i].layer);
      if (!sp) continue;
      var doc = sp.value;
      doc.text = translated;
      sp.setValue(doc);
  ...
  function buildUI(thisObj){
    ...
    var btnSnapshotMarker = createFlatButton(rowSnapshot, "\u25B2", "Snapshot Marker - Select a text layer (works in main comp or precomp), set the playhead to the desired frame, then click to set as the preferred time for Smart Scan.", 22);
    ...
    // Composition footer: one left TMS action slot (AE centers 2nd child if 1st is hidden), Readme right.
    ...
    var lblNewVersion = footerLeftSlot.add("statictext", undefined, "New version available");
    lblNewVersion.alignment = ["left", "center"];
    ...
    compFooterSpacer.alignment = ["fill", "bottom"];
    var btnReadmeComp = compositionFooter.add("statictext", undefined, "Readme");
    btnReadmeComp.alignment = ["right", "center"];

    function syncFooterTmsActionButton() {
      try {
        if (TMS_PROVIDER === "phrase") {
          btnFooterTmsAction.text = "AI Translation";
          btnFooterTmsAction.minimumSize = [120, 24];
          btnFooterTmsAction.preferredSize = [120, 24];
          btnFooterTmsAction.helpTip = "Pre-translate with Phrase Language AI, then duplicate selected compositions with translated text (AI Translation).";
          btnFooterTmsAction.visible = true;
        } else if (TMS_PROVIDER === "crowdin_team" || TMS_PROVIDER === "crowdin_enterprise") {
          btnFooterTmsAction.text = "Pseudo Translation";
          btnFooterTmsAction.minimumSize = [140, 24];
          btnFooterTmsAction.preferredSize = [140, 24];
          btnFooterTmsAction.helpTip = "Test layout only: Crowdin adds accent-style characters (e.g. CONNECT \u2192 \u00C7\u00D4\u00D1\u00D1\u00C9\u00C7\u0162), not real Spanish/French. For real translations, use Import Selected after translating in Crowdin.";
          btnFooterTmsAction.visible = true;
        } else {
          btnFooterTmsAction.visible = false;
        }
    ...
    stylePanelLinkText(btnReadmeComp);
    ...
    function updateTmsUiLabels() {
      ...
      try {
        if (cbSegmentation) {
          ...
          if (TMS_PROVIDER === "phrase") {
            cbSegmentation.helpTip = "When enabled, long lines may be split into multiple segments in Phrase TMS.";
          } else {
    ...
    btnReadmeComp.addEventListener("click", function () { showReadmeDialog(); });
    ...
    btnOpenCrowdin.addEventListener("click", function () {
      ...
      openUrl("https://crowdin.com/project/" + encodeURIComponent(slug));
    });
    ...
    function runFooterAiTranslation() {
      ...
      var importSteps = phraseSteps;
      var totalSteps = phraseSteps + importSteps;
      if (totalSteps < 1) totalSteps = 1;

      var popup = showCrowdinAeProgressPopup("AI Translation");
      function statusProxy(msg) {
        updateProgressPopup(popup, popup && popup.bar ? popup.bar.value : 0, msg || "");
        setStatus(msg);
      }
      function popupSetProgress(current, total) {
        if (!popup || !popup.bar) return;
        ...
        if (frac < 0) frac = 0;
        if (frac > 1) frac = 1;
        updateProgressPopup(popup, frac * 100, popup.statusLabel ? popup.statusLabel.text : "");
      }

      var step = 0;
      updateProgressPopup(popup, 0, "AI Translation: preparing…");
      ...
      for (var ac = 0; ac < comps.length; ac++) {
        ...
        for (var li = 0; li < selected.length; li++) {
          ...
          var fromPct = (step / totalSteps) * 100;
          var toPct = ((step + 1) / totalSteps) * 100;
          var msgPhrase = "AI Translation: sending to Phrase — " + compLabel + " \u2192 " + langLabel + "…";
          pulseProgressPopup(popup, fromPct, fromPct + (toPct - fromPct) * 0.15, msgPhrase, 6);
          statusProxy(msgPhrase);
          popupSetProgress(step, totalSteps);
          if (!phraseRunAutoTranslate(fk, [lang.id], statusProxy)) {
            if (popup && popup.win) try { popup.win.close(); } catch (eAt) {}
            return;
          }
          step++;
          popupSetProgress(step, totalSteps);
          pulseProgressPopup(popup, toPct - (toPct - fromPct) * 0.1, toPct, "AI Translation: received from Phrase — " + langLabel, 4);
        }
      }

      statusProxy("AI Translation: duplicating compositions…");
      var result = importSelectedCompositionsWithProgress(statusProxy, popupSetProgress, {
      ...
      if (!result.ok) {
        if (popup && popup.win) try { popup.win.close(); } catch (eAt2) {}
        return;
      }
      statusProxy("AI Translation complete. " + result.done + " composition(s) created.");
      updateProgressPopup(popup, 100, "AI Translation complete.");
      if (popup && popup.win) try { popup.win.close(); } catch(eC4) {}
    }
    ...
    function runFooterPseudoTranslation() {
      ...
      try {
        ...
        var pseudoSteps = 1;
        var importSteps = comps.length * selected.length;
        var totalSteps = pseudoSteps + importSteps;
        if (totalSteps < 1) totalSteps = 1;

        popup = showCrowdinAeProgressPopup("Pseudo Translation");
        function statusProxy(msg) {
          updateProgressPopup(popup, popup && popup.bar ? popup.bar.value : 0, msg || "");
          setStatus(msg);
        }
        ...
        updateProgressPopup(popup, 2, "Pseudo Translation: building on Crowdin (1–3 min)…");
        popupSetProgress(0, totalSteps);
        try { app.refresh(); } catch (eRf0) {}

        var pseudoData = crowdinRunPseudoTranslate(fileKeys, statusProxy);
        if (!pseudoData || !hasAnyPseudoFiles(pseudoData.files)) {
          return;
        }
        popupSetProgress(1, totalSteps);
        updateProgressPopup(popup, Math.round((1 / totalSteps) * 100), "Pseudo Translation: duplicating compositions…");

        var result = importSelectedCompositionsWithProgress(statusProxy, popupSetProgress, {
          progressOffset: pseudoSteps,
          progressTotal: totalSteps,
        ...
        statusProxy("Pseudo-localization complete. " + result.done + " composition(s) created.");
        ...
        if (popup && popup.win) try { popup.win.close(); } catch (ePsFin) {}
        try { pal.layout.layout(true); } catch (eLay) {}
      }
    }

    btnFooterTmsAction.onClick = function () {
    ...
    function doOauthThenProjects(mode) {
      var started = oauthConnect(setStatus, mode, function(ok) {
        if (!ok) return;
        STATE.projects = loadProjects(setStatus);
